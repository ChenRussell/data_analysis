...
public class MinimizingMakeChangeFitnessFunction extends FitnessFunction {
	...
	public MinimizingMakeChangeFitnessFunction(int a_targetAmount) {
		if (a_targetAmount < 1 || a_targetAmount >= MAX_BOUND) {
			throw new IllegalArgumentException("Change amount must be between 1 and " + MAX_BOUND + " cents.");
		}
	 ...
	 * value, the more fit the instance. This method should always return the same
	 * fitness value for two equivalent Chromosome instances.
	 *
	 * @param a_subject
	...
	public double evaluate(IChromosome a_subject) {
		 ...
		 * 打印显示并没有false
		 */
		System.out.println("defaultComparation: " + defaultComparation);
	...
	/**
	 * Calculates the penalty to apply to the fitness value based on the ammount of
	...
	/**
	 * Retrieves the number of coins represented by the given potential solution at
	 * the given gene position.
	 *
	 * @param a_potentialSolution
	 *            the potential solution to evaluate
	 * @param a_position
